...
export class SunischitLaabh {
    ...
    async fillPlanDetails(testcaseID: string) {
        ...
        await dropdown.selectAntDropdown('MaturityPayoutMode', data.Maturity_Payout);
        await dropdown.selectAntDropdown('MatPayoutPeriod', data.Maturity_Payout_Period);
        await dropdown.selectAntDropdown('Deathbenefit', data.Death_Benefit);

        if(data.Death_Benefit.includes("Life Plus Option") && !data.Plan_Name.includes("POS")) {
            await dropdown.selectAntDropdown('DeathPayoutMode', data.Death_Payout);
        }
        
        await dropdown.selectAntDropdown('SAMultiplier', data.SA_Multiplier);
